...
const Conductor: React.FC = () => {
  ...
  useEffect(() => {
    // Listen for changes in the Conductors collection
    ...
    // Get the last bus number for the next new conductor
  ...
  const addConductor = async (data: any) => {
    try {
      ...
      // Add the conductor's data to Firestore
      ...
      setNextBusNumber((prev) => prev + 1); // Increment the bus number for the next conductor
      reset();
  ...
  const deleteConductor = async (id: string, email: string) => {
    try {
      ...
      // Delete the Firebase Authentication user if it's the same as the conductor's email
